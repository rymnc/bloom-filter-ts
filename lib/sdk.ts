...
export class BloomFilter {
    ...
    constructor(m: number, n: number, permittedError: number = 0.15, hashingFunctions: Array<HashingFunctions>) {
        this.m = m;
        this.n = n;
        this.k = (m/n)*Math.log(2);
        const error = Math.pow((1 - Math.exp((-this.k*n)/m)), this.k);
        if(error > permittedError) throw new Error(`Permitted error: ${permittedError}, found error: ${error}`)
